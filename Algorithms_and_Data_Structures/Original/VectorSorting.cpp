...
void quickSort(vector<Bid>& bids, int begin, int end) {
	...
	// recursively call quickSort on using midpoint value (begin to mid)
	quickSort(bids, begin, mid);
	// recursively call quickSort on using midpoint value mid +1 to end)
...
void selectionSort(vector<Bid>& bids) {
	// index to current minimum
	int min;
	// pos is the position within the bids that marks sorted/unsorted bids
	for (unsigned pos = 0;pos < bids.size(); ++pos) {
		min = pos;
		//inner loop to compare current bid title to other bids, and sort
		...
		if (min != pos) {
			...
			//swap the bids to sort
			Bid temp = bids.at(pos);
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 4:
            ...
            cout << bids.size() << " bids sorted quickly" << endl;
